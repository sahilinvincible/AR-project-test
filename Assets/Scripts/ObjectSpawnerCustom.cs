...
*/
        ...
        foreach (var annotation in AnnotationsList)
        {
            Debug.Log(AnnotationsList.Count + "  inside instnidex" + currentIndex);
           // Debug.Log("Loop iteration: " + currentIndex + " with annotation title: " + annotation.title);


            Debug.Log(" buttons are instantiated");
            int index = currentIndex;
            var antbutton = Instantiate(annotationsMenuButtonPrefabContainer, Vector3.zero, Quaternion.identity);
            annotationButtonList.Add(antbutton);
            UnityEngine.UI.Button buttonClick = antbutton.transform.GetComponent<UnityEngine.UI.Button>();
            ...
            // buttonClick.onClick.AddListener(() => menuManager.SetObjectToSpawn(objectIndex));
            antbutton.GetComponentInChildren<TextMeshProUGUI>().text = AnnotationsList[index].title;// Annotationslist[index].title;
            // button.GetComponent<TextMeshProUGUI>().text = "Clicked!";
            antbutton.transform.SetParent(uiAnnotationContent.transform, false);
            //annotationButtonList.Add(antbutton);
    ...
    public void ClearButtons()
    {
        ...
        {
            foreach (GameObject button in annotationButtonList)
            {
                Destroy(button);
            }
        }
        annotationButtonList.Clear();
    }
    ...
    IEnumerator RotateObject(GameObject obj, Vector3 buttonWorldPosition, Camera camera, float duration)
    {
       ...
       // Debug.Log("clopoint" + ClosestPoint);
